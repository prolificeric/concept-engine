import { Concept } from '..';
import { applyAmpersandOperator, applyColonOperator } from './operators';
...
describe('colon operator', () => {
  describe('shallow compound', () => {
    test('sorts the parts alphabetically after colon', () => {
      expect(
        applyColonOperator(Concept.createCompound([':', 'b', 'a', 'c'])).key,
      ).toEqual(': a b c');
    });
  });
